...
export class ChatTitleGeneratorService {
  ...
  public async createChatTitle(chat: Chat, input: string) {
    ...
    const response = await this.modelProviderService.generateResponse(
      1,
      'gpt-4o-mini',
      `На основе первого сообщения из диалога, сгенерируй одно короткое (не более 5 слов) название, которое точно отражает основную тему чата. Название должно быть на русском языке. Вот первое сообщение - "${
